...
{
    QSqlQuery query;
    string s = "SELECT id, pass, lock FROM users WHERE name = '";
    s += username.c_str();
    s += "'";
    // qDebug() << s.c_str();
    query.exec(s.c_str());
    if(query.next())                                // if user found
    {
        ...
        int lock = query.value(2).toInt();
        if(lock != 0)return -2;                                 // is user locked?
...
vector<string> Database::getChatMessages()
{
    ...
    while(query.next())
    {
        int sent = query.value(1).toString().toInt();
        string writer = getUserName(sent);
        string text = query.value(3).toString().toStdString();
        strings.push_back("<" + writer + ">: " + text);
    };
	return strings;
}
